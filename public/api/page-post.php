...
if ($action == "insert") {
	...
	$fields = ['id', 'title', 'body', 'tag', '_created', '_edited'];
	$cols = [];
	$vals = [];
	$types = '';
	$params = [];
	...
	foreach ($fields as $field) {
		if (isset($data[$field]) && $data[$field] !== null && $data[$field] !== '') {
			$cols[] = "`$field`";
			$vals[] = '?';
			if (in_array($field, ['_created', '_edited'])) {
				$types .= 'i';
			} else {
				$types .= 's';
			}
	...
	$sql = "INSERT INTO `page` (" . implode(',', $cols) . ") VALUES (" . implode(',', $vals) . ")";
	...
	$sql = "UPDATE `page` SET " . implode(',', $sets) . " WHERE `id`=?";
	$stmt = $conn->prepare($sql);
	call_user_func_array([$stmt, 'bind_param'], $params);
	if (!$stmt->execute()) {
		$stmt->close();
...
echo file_get_contents("http://" . $_SERVER['SERVER_NAME'] . "/api/page.php?id=" . $id);
